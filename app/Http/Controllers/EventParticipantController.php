...
class EventParticipantController extends Controller
{
    ...
    {
        try {
            $eventParticipants = $this->eventParticipantRepository->getAll($request->search, $request->limit, true);
            return ResponseHelper::jsonResponse(true, 'Data Pendaftar Event Berhasil Diambil', EventParticipantResource::collection($eventParticipants));
        } catch (\Throwable $th) {
    ...
    public function getAllPaginated(Request $request)
    {
        ...
        try {
            $eventParticipants = $this->eventParticipantRepository->getAllPaginated($request['search'] ?? null, $request['row_per_page']);
            return ResponseHelper::jsonResponse(true, 'Data Pendaftar Event Berhasil Diambil', PaginateResource::make($eventParticipants));
        } catch (\Throwable $th) {
            return ResponseHelper::jsonResponse(false, $th->getMessage(), null, 500);
        }
    }
     ...
     public function store(EventParticipantStoreRequest $request)
    {
        ...
        try {
            $eventParticipants = $this->eventParticipantRepository->create($data);

            return ResponseHelper::jsonResponse(
                true,
                'Data Pendaftar Event Berhasil Disimpan',
